...
Expected: empty status and functioning old renderer fed by new immutable live data. Continue to this plan's `## Phase Integration Gate`. Do not start Phase C until that gate's final fresh `origin/main` verification succeeds.
...
4. Pin the exact phase tip, push that phase branch to `origin`, and bind the
   ...
   git push origin feature/level1-phase-b
   ...
   if ((git rev-parse origin/feature/level1-phase-b) -ne $expectedPhaseTip) {
     throw 'Remote phase branch does not match pinned phase tip'
   }
   Write-Output "Reviewed phase tip SHA: $expectedPhaseTip"
   ```

   Review the full `origin/main...feature/level1-phase-b` diff. Retain both printed SHA values with the review record.
...
5. Rebind the retained review values literally and integrate:
   ...
   $localPhaseTip = git rev-parse feature/level1-phase-b
